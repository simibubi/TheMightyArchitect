...
public abstract class SymmetryElement {
	...
	public static SymmetryElement fromNBT(NBTTagCompound nbt) {
		...
		case POINT:
			element = new SymmetryPoint(pos);
			break;
		case AXIS:
			element = new SymmetryAxis(pos);
			break;
		case PLANE:
	...
	protected IBlockState flipD1(IBlockState in) {
		for (IProperty<?> property : in.getPropertyKeys()) {
			...
			if (property instanceof PropertyDirection) {
				...
				case EAST:  return in.withProperty((PropertyDirection) property, EnumFacing.NORTH);
				case NORTH: return in.withProperty((PropertyDirection) property, EnumFacing.EAST);
				case SOUTH: return in.withProperty((PropertyDirection) property, EnumFacing.WEST);
				case WEST:  return in.withProperty((PropertyDirection) property, EnumFacing.SOUTH);
				default: break;
				}
			}
	...
	protected IBlockState flipD2(IBlockState in) {
		for (IProperty<?> property : in.getPropertyKeys()) {
			...
			if (property instanceof PropertyDirection) {
				switch ((EnumFacing) in.getProperties().get(property)) {
				case EAST:  return in.withProperty((PropertyDirection) property, EnumFacing.SOUTH);
				case NORTH: return in.withProperty((PropertyDirection) property, EnumFacing.WEST);
				case SOUTH: return in.withProperty((PropertyDirection) property, EnumFacing.EAST);
				case WEST:  return in.withProperty((PropertyDirection) property, EnumFacing.NORTH);
				default: break;
				}
			}
