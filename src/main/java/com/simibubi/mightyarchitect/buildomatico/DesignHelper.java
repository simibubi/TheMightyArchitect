package com.simibubi.mightyarchitect.buildomatico;

import java.util.List;

import com.simibubi.mightyarchitect.buildomatico.DesignPicker.DesignCache;
import com.simibubi.mightyarchitect.buildomatico.model.sketch.Design;
import com.simibubi.mightyarchitect.buildomatico.model.sketch.Design.DesignInstance;
import com.simibubi.mightyarchitect.buildomatico.model.sketch.Design.Style;
import com.simibubi.mightyarchitect.buildomatico.model.sketch.Design.Type;
import com.simibubi.mightyarchitect.buildomatico.model.sketch.Roof;

import net.minecraft.util.math.BlockPos;

public class DesignHelper {

	/**
	 * Creates a closed room around the specified cuboid using 4 walls and 4
	 * corners
	 */
	static void addCuboid(DesignCache cache, List<DesignInstance> designList, Style weight, BlockPos start, BlockPos size) {

		Design facadeA = null;
		Design facadeB = null;
		int widthX = size.getX();
		int height = size.getY();
		int widthZ = size.getZ();

		if (widthX <= widthZ)
			facadeA = cache.pick(weight, Type.FACADE, widthX - 2, height);
		if (widthZ <= widthX)
			facadeB = cache.pick(weight, Type.FACADE, widthZ - 2, height);
		Design wallA = (facadeA != null) ? facadeA : cache.pick(weight, Type.WALL, widthX - 2, height);
		Design wallB = (facadeB != null) ? facadeB : cache.pick(weight, Type.WALL, widthZ - 2, height);
		Design corner = cache.pick(weight, Type.CORNER, height);

		BlockPos cornerZ = start.add(0, 0, widthZ - 1);
		BlockPos cornerXZ = start.add(widthX - 1, 0, widthZ - 1);
		BlockPos cornerX = start.add(widthX - 1, 0, 0);

		if (wallA != null && wallB != null && corner != null) {
			designList.add(wall(wallA, cornerZ, cornerXZ, height));
			designList.add(wall(wallB, cornerXZ, cornerX, height));
			designList.add(wall(wallA, cornerX, start, height));
			designList.add(wall(wallB, start, cornerZ, height));
			designList.add(corner(corner, start, 135, height));
			designList.add(corner(corner, cornerZ, 45, height));
			designList.add(corner(corner, cornerXZ, -45, height));
			designList.add(corner(corner, cornerX, -135, height));
		}

	}

	/**
	 * Adds a wall in the specified design between the corner points facing z+
	 * when x1 < x2
	 */
	static DesignInstance wall(Design design, BlockPos corner1, BlockPos corner2, int height) {
		int xDiff = corner2.getX() - corner1.getX();
		int zDiff = corner2.getZ() - corner1.getZ();
		int xStep = (int) Math.signum(xDiff);
		int zStep = (int) Math.signum(zDiff);
		corner1 = corner1.add(xStep, 0, zStep);
		int width = Math.abs((xDiff == 0) ? zDiff : xDiff) - 1;
		int rotation = ((xDiff == 0) ? ((zDiff > 0) ? 90 : -90) : ((xDiff > 0) ? 0 : 180));
		return design.create(corner1, rotation, width, height);
	}

	/**
	 * Creates a corner of the specified design. Valid angles: 45, 135, -135,
	 * -45,
	 */
	static DesignInstance corner(Design design, BlockPos pos, int angle, int height) {
		return design.create(pos, angle + 45, height);
	}

	/**
	 * Creates a trim of the specified design. Valid angles: 0, 90, 180, -90
	 */
	static DesignInstance trim(Design design, BlockPos pos, int angle, int height) {
		return design.create(pos, angle, height);
	}

	/**
	 * Creates a roof of the specified design. Valid angles: 0, 90, 180, -90
	 */
	static DesignInstance roof(Design design, BlockPos pos, int angle, int depth) {
		return ((Roof) design).create(pos, angle, depth);
	}

	/**
	 * Creates a roof with two facades sitting on the shorter sides of the
	 * cuboid
	 */
	static void addDoubleRoof(DesignCache cache, List<DesignInstance> designList, BlockPos start, BlockPos size) {
		boolean south = size.getZ() < size.getX();
		int depth = south ? size.getX() : size.getZ();
		int width = south ? size.getZ() : size.getX();

		BlockPos cornerZ = start.add(0, 0, size.getZ() - 1);
		BlockPos cornerXZ = start.add(size.getX() - 1, 0, size.getZ() - 1);
		BlockPos cornerX = start.add(size.getX() - 1, 0, 0);

		Design roof = cache.pick(Style.ANY, Type.ROOF, width, 0);
		if (roof != null) {
			designList.add(roof(roof, south ? start : cornerX, south ? 90 : 180, depth - 4));
			designList.add(roof(roof, south ? cornerXZ : cornerZ, south ? -90 : 0, depth - 4));
		}

	}
}
